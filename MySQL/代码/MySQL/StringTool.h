...
namespace StringTool
{
	...
	inline BOOL EncodeGB2312(const char* szSrc, char* pBuf, int cbBufLen, BOOL bUpperCase)
	{
		...
		while(*pSrc && cbDest < cbBufLen - 1)
		{
			...
			// isalpha判断是否为英文字母，为小写返回2，为大写返回1；isdigit判断是否为0-9;
			{
				*pDest = c;
				++pDest;
				++cbDest;
	...
	// Encode编码;
	inline void UrlEncode(IN const char *szUrl,OUT char *buf)
	{
	...
	// UTF-8到GB2312的转换;
	inline void  Utf8ToGb2312(IN const char* utf8,OUT char *szGB)
	{
